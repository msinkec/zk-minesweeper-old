...
async function zokratesProof(mineField, mineFieldCommit, x, y, mineHit, neighborMineCount) {
...
async function mineFieldMimc(mineField) {
  const mimc7 = await buildMimc7();
  return mimc7.F.toString(mimc7.hash(BigInt(mineField), 0));
}

async function mineFieldArrToInt(mineFieldArr) {
  let binStr = '0b';
